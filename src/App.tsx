...
const BRAND = 'S3 CLEANING SERVICE';
...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen bg-[#020c1b] text-white selection:bg-sky-500/30 overflow-x-hidden">
      <CustomCursor />
      <TouchRippleLayer />
      <MobileMenu isOpen={menuOpen} onClose={() => setMenuOpen(false)} />

      {/* BG Glows */}
      <div className="absolute top-0 right-0 -translate-y-1/4 translate-x-1/4 w-[500px] h-[500px] bg-sky-500 rounded-full blur-[160px] opacity-[0.08] pointer-events-none" />
      <div className="absolute bottom-0 left-0 translate-y-1/4 -translate-x-1/4 w-[400px] h-[400px] bg-cyan-400 rounded-full blur-[130px] opacity-[0.06] pointer-events-none" />

      {/* ── NAVBAR ── */}
      <nav className={`fixed top-0 left-0 right-0 z-50 px-3 py-3 sm:px-4 sm:py-4 md:px-8 md:py-6 transition-all duration-300 ${scrolled ? 'bg-[#020c1b]/80 backdrop-blur-xl' : ''}`}>
        <div className="max-w-7xl mx-auto flex items-center justify-between bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl md:rounded-2xl px-4 py-3 md:px-6 md:py-4 shadow-2xl">
          <div className="font-display font-bold text-xl sm:text-2xl tracking-widest uppercase text-sky-400">{BRAND}</div>
          ...
          <a href="#contact" className="hidden md:inline-block bg-sky-500 text-white px-8 py-3 rounded-full font-bold text-xs uppercase tracking-widest hover:bg-sky-400 transition-all duration-300 active:scale-95">
            Book a Clean
          </a>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-10 md:gap-16 items-center w-full">
          <div className="relative z-10">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
              className="inline-block bg-sky-500/10 border border-sky-500/30 text-sky-400 text-xs font-bold uppercase tracking-[0.2em] px-5 py-2 rounded-full mb-6">
            ...
            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
              ...
              Your Space,{' '}<br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-cyan-400">Spotless.</span>
              <br />Always.
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.2 }}
              ...
              Premium home and office cleaning services across Pune. Eco-friendly products, trained staff, and a spotless guarantee — every single time.
            </motion.p>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.4 }}
              ...
              <a href="#contact" className="w-full sm:w-auto bg-sky-500 text-white px-8 sm:px-10 py-4 sm:py-5 rounded-full font-bold text-sm uppercase tracking-widest hover:bg-sky-400 transition-all duration-300 active:scale-95 hover:shadow-[0_0_20px_rgba(14,165,233,0.4)] flex items-center justify-center gap-3">
                Book Free Estimate →
              </a>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.6 }}
              ...
              {[['500+','Happy Clients'], ['5★','Rated on Google'], ['3+','Years in Pune']].map(([n, l]) => (
                <div key={l} className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-4 py-2">
                  <span className="text-sky-400 font-display font-bold text-sm">{n}</span>
          ...
          <div className="relative hidden sm:block">
            <motion.div initial={{ opacity: 0, scale: 0.95, filter: 'blur(10px)' }} animate={{ opacity: 1, scale: 1, filter: 'blur(0px)' }} transition={{ duration: 1.2, delay: 0.2 }}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[#020c1b] via-transparent to-transparent z-10 opacity-60 pointer-events-none" />
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: 0.8 }}
              ...
              <div className="w-12 h-12 bg-sky-500/10 rounded-xl flex items-center justify-center text-sky-400 border border-sky-500/20 text-2xl">🏠</div>
              <div>
      ...
      {/* ── MARQUEE ── */}
      <section className="marquee-section w-full overflow-hidden bg-sky-500/5 backdrop-blur-md border-y border-sky-500/10 py-3 sm:py-4 md:py-6 relative z-20">
        <div className="marquee-track whitespace-nowrap">
          {[0, 1].map(copy => (
            <span key={copy} aria-hidden={copy === 1} className="marquee-content font-anton text-4xl sm:text-5xl md:text-6xl text-stroke-heavy uppercase tracking-tight">
              DEEP CLEANING<span className="mx-4 sm:mx-6 text-sky-400"> • </span>
              OFFICE & COMMERCIAL<span className="mx-4 sm:mx-6 text-sky-400"> • </span>
              SOFA & CARPET<span className="mx-4 sm:mx-6 text-sky-400"> • </span>
              MOVE IN / MOVE OUT<span className="mx-4 sm:mx-6 text-sky-400"> • </span>
              ECO-FRIENDLY<span className="mx-4 sm:mx-6 text-sky-400"> • </span>
              POST-CONSTRUCTION<span className="mx-4 sm:mx-6 text-sky-400"> • </span>
            </span>
          ))}
        </div>
      </section>
      ...
      <section id="about" className="relative py-16 sm:py-20 md:py-32 px-4 sm:px-6">
        <div className="absolute top-1/2 left-1/4 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] bg-sky-500/15 rounded-full blur-[100px] pointer-events-none" />
          ...
          <div>
            <Reveal direction="left">
              <h2 className="font-display text-4xl sm:text-5xl md:text-7xl font-bold uppercase tracking-tighter mb-6 md:mb-8">
                Pune's Most<br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-cyan-400">Trusted</span><br />
                Cleaners
              </h2>
            </Reveal>
            <Reveal delay={0.15} direction="left">
              <p className="text-white/50 text-base sm:text-lg font-light leading-relaxed mb-6 md:mb-8">
                S3 Cleaning Service was born in Pune with one simple belief — everyone deserves a spotless space without the hassle. We serve homes, offices, and commercial spaces across Pune, Pimpri-Chinchwad, Kothrud, Baner, Wakad, and beyond. Our eco-certified team delivers deep, thorough, and reliable cleaning — every visit, every time.
              </p>
            </Reveal>
            <Reveal delay={0.25} direction="left">
              <a href="#contact" className="inline-block bg-sky-500/10 border border-sky-500/30 text-sky-400 px-6 sm:px-8 py-3 sm:py-4 rounded-full font-bold text-xs uppercase tracking-widest hover:bg-sky-500/20 transition-all duration-300 active:scale-95">
                Get a Free Quote
              </a>
            </Reveal>
          </div>
            ...
            <Reveal direction="left">
              <div>
                <h2 className="font-display text-4xl sm:text-5xl md:text-7xl font-bold uppercase tracking-tighter">
                  Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-cyan-400">Services</span>
                </h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                {EXTRA_SERVICES.map((s) => (
                  <a key={s.name} href="#contact"
                    className="group flex items-start gap-3 sm:gap-4 bg-white/[0.02] hover:bg-white/[0.05] border border-white/8 hover:border-sky-500/30 rounded-2xl p-4 sm:p-5 transition-all duration-300 hover:shadow-[0_0_20px_rgba(14,165,233,0.08)] cursor-pointer">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-sky-500/10 border border-sky-500/20 rounded-xl flex items-center justify-center text-xl shrink-0 group-hover:bg-sky-500/20 group-hover:border-sky-400/40 transition-all duration-300">
                      {s.icon}
                    </div>
                    <div className="min-w-0">
                      <h4 className="font-display font-bold text-sm sm:text-base uppercase tracking-wide text-white group-hover:text-sky-300 transition-colors duration-300 leading-tight mb-1">{s.name}</h4>
        ...
        <div className="max-w-7xl mx-auto">
          <Reveal>
            <div className="mb-10 md:mb-16 text-center">
              <h2 className="font-display text-4xl sm:text-5xl md:text-7xl font-bold uppercase tracking-tighter">
                Why Choose <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-cyan-400">S3?</span>
              </h2>
      ...
      <section id="contact" className="relative py-16 sm:py-20 md:py-32 px-4 sm:px-6">
        <div className="absolute top-1/2 right-0 -translate-y-1/2 translate-x-1/4 w-[500px] h-[500px] bg-sky-500/15 rounded-full blur-[120px] pointer-events-none" />
        <div className="max-w-4xl mx-auto relative z-10 text-center">
          <Reveal>
            <div className="inline-block bg-sky-500/10 border border-sky-500/30 text-sky-400 text-xs font-bold uppercase tracking-[0.2em] px-5 py-2 rounded-full mb-6">
              Free Estimate — No Obligation
            </div>
          </Reveal>
          <Reveal delay={0.1}>
            <h2 className="font-display text-4xl sm:text-5xl md:text-6xl lg:text-8xl font-bold uppercase tracking-tighter mb-6">
              Ready for a <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-cyan-400">Spotless</span> Space?
            </h2>
          </Reveal>
          <Reveal delay={0.2}>
          ...
          <Reveal delay={0.3}>
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-4 sm:gap-6 mb-12">
              <a href="tel:+917xxxxxx000" className="bg-sky-500 text-white px-8 sm:px-10 py-4 sm:py-5 rounded-full font-bold text-sm uppercase tracking-widest hover:bg-sky-400 transition-all duration-300 active:scale-95 hover:shadow-[0_0_20px_rgba(14,165,233,0.4)] flex items-center justify-center gap-3">
                📞 Call Us Now
              </a>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
            {[
              ...
              { icon: '✉️', label: 'hello@s3cleaning.in' },
              { icon: '📍', label: 'Baner, Pune, Maharashtra' },
            ].map((item, i) => (
              <div key={i}>
              <Reveal delay={0.1 + i * 0.1}>
                <div className="bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-2xl p-5 flex flex-col items-center gap-3 hover:border-sky-500/30 transition-all duration-300">
      ...
      <footer className="relative overflow-hidden border-t border-white/10">
        ...
        <div className="relative overflow-hidden" style={{ height: '52vh', minHeight: '220px' }}>
          ...
          <div className="absolute inset-0 pointer-events-none" style={{
            background: 'radial-gradient(ellipse 55% 75% at 50% 55%, rgba(14,165,233,0.13) 0%, rgba(6,182,212,0.05) 45%, transparent 75%)'
          }} />

          {/* The text — solid, height-filling, bleeding off sides */}
          <div className="absolute inset-0 flex items-center justify-center">
            <h2
              className="font-display font-bold uppercase whitespace-nowrap select-none leading-none"
              style={{
                fontSize: 'clamp(5rem, 26.5vw, 32rem)',
                letterSpacing: '-0.015em',
                lineHeight: 0.88,
                background: 'linear-gradient(175deg, #ffffff 0%, #bae6fd 55%, rgba(186,230,253,0.7) 100%)',
        ...
        <div className="border-t border-white/[0.07] px-4 sm:px-6 py-5 sm:py-7">
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <div className="font-display font-bold text-sm tracking-[0.2em] uppercase text-sky-400/80 mb-0.5">{BRAND} Cleaners · Pune</div>
              <p className="text-white/20 text-xs tracking-wide">Your Space, Spotless. Always.</p>
            </div>
            <div className="flex flex-col items-start sm:items-end gap-1 text-white/20 text-xs">
              <p>Baner, Pune, Maharashtra — 411 045</p>
              <p>+91 7XX XXX XX00 · hello@s3cleaning.in</p>
              <p className="mt-1 text-white/12">© {new Date().getFullYear()} S3 Cleaning Service. All rights reserved.</p>
            </div>
          </div>
        </div>
      </footer>
